...
/* use if then to limit flowers based on price*/
...
const zoneFlowers = () => {
    ...
    // use if and && to filter by zones and includes to access zone info
for (const flower of flowers){
...
const orangeFlowers = () => {
    ...
    // use includes to find all orange flowers
    for (const flower of flowers){
...
const flowersAsHTML = () => {
    ...
    /*
       use nested for loops to add the html text and formatting to the string
    */
   //first create the article with the type heading//

for (const flower of flowers){
    flowersHTMLString += (`<article> \n 
    \t <h1> ${flower.type} </h1> \n`)

//colors heading and sections within the initial flower loop//

    for (const color of flower.colors){
        flowersHTMLString += (` \t <h2> Colors </h2> \n
        \t <section> ${color} </section> \n`) }

//zone heading and sections within the initial flower loop//
